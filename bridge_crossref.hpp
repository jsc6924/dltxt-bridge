...
class CrossrefService {
        ...
        for (const auto& workspace_folder_uri : workspace_folders) {
            ...
            while (!error && iterator != end) {
                ...
                if (iterator->is_directory(error)) {
                    if (!error && is_hidden_name(current_path)) {
                ...
                if (!error && iterator->is_regular_file(error) && !is_hidden_name(current_path) && is_text_file(current_path)) {
    ...
    static bool path_is_within_root(const std::filesystem::path& file_path, const std::filesystem::path& root_path) {
        const std::string file_string = file_path.lexically_normal().generic_string();
        std::string root_string = root_path.lexically_normal().generic_string();
        ...
        return file_string == root_path.lexically_normal().generic_string()
            || (file_string.size() > root_string.size() && file_string.rfind(root_string, 0) == 0);
    }
    ...
    void replace_document_unlocked(const std::string& file_path, const std::vector<bridge_text::ParsedPair>& pairs) {
        ...
        for (const auto& pair : pairs) {
            ...
            line_infos_.emplace(line_id, LineInfo{file_path, pair.original_line_index, pair.original.text, pair.translated.text});
    ...
    static void ingest_document(
        ...
        const std::string file_path = document.getFilePath().string();
        ...
        try {
            ...
            if (!pairs.empty()) {
                ...
                for (const auto& pair : pairs) {
                    ...
                    ids.push_back(line_id);
                    state.line_infos.emplace(line_id, LineInfo{file_path, pair.original_line_index, pair.original.text, pair.translated.text});
    ...
    std::vector<LineSearchResult> search_line(
        ...
        const std::string current_base_name = std::filesystem::path(current_file_path).filename().string();
...

        {
            ...
            for (const auto& candidate : raw_candidates) {
                ...
                const std::string candidate_base_name = std::filesystem::path(line_info.file_name).filename().string();
                if (candidate_base_name == current_base_name
...
public:
    ...
    void update_parser_config(
        const json& parser_payload,
        const std::vector<std::string>& workspace_folders,
        const std::vector<bridge_documents::TextDocument>& open_documents,
        boost::asio::thread_pool& pool) {
        ParserState parser_state{
            bridge_text::regex_config_from_json(parser_payload),
            parser_payload.dump(),
        };
    ...
    void schedule_open_document_update(bridge_documents::TextDocument document, boost::asio::thread_pool& pool) {
        ...
        ParserState parser_state;
        {
            ...
            const auto version_it = open_document_versions_.find(document.getFilePath().string());
        ...
        boost::asio::post(pool, [this, document = std::move(document), parser_state = std::move(parser_state)]() mutable {
            ...
            try {
                ...
                replace_document_unlocked(file_path, pairs);
    ...
    void schedule_closed_document_refresh(const std::string& uri, boost::asio::thread_pool& pool) {
        ...
        ParserState parser_state;
        {
            ...
            if (!is_in_workspace_unlocked(file_path)) {
                erase_file_unlocked(file_path.string());
                return;
            }
        ...
        boost::asio::post(pool, [this, file_path, parser_state = std::move(parser_state)]() mutable {
            ...
            try {
                ...
                replace_document_unlocked(file_path_string, pairs);
    ...
    json search_json(
        ...
        const std::string current_file_path = current_document.getFilePath().string();
        ...
        for (const auto& pair : current_pairs) {
            ...
            auto refs = search_line(pair.original.text, threshold, limit, current_file_path, pair.original_line_index, &exact_count);
